...
/**
    @file
    @brief CCM モードによる認証付き復号化を利用するための API の宣言
 */
...
/**
    @brief      CCM 暗号利用モードによる認証付き復号化を行うクラスです。
    ...
    実際の使用には Aes128CcmDecryptor などを利用してください。
    ...
    CCM モードの復号化ではブロック暗号の復号化処理ではなく、暗号化処理が用いられることに注意してください。
...
class CcmDecryptor
{
    NN_DISALLOW_COPY(CcmDecryptor<BlockCipher>);
...
public:
     ...
     */
    CcmDecryptor() NN_NOEXCEPT {}

    /**
        @brief      CCM モードによる認証付き復号化の処理を初期化します
        ...
        Nonce、追加認証データ、暗号文データ、MAC のサイズは暗号化で使用したものと同じものを使用してください。
    ...
    /**
        ...
        - UpdateAad によって入力されるデータの合計が Initialize() で指定された
          aadSize を超えない。

        @post
        - 追加認証データとして入力されたサイズが aadSize に達したとき、Initialize() で指定された dataSize が 0 でない場合、インスタンスは復号化処理可能状態になる。
    ...
    /**
        ...
        @post
        - 復号化データとして入力されたサイズの合計が Initialize() で指定された dataSize に達した場合、インスタンスはデータ入力完了状態になる。
        - pDst に復号化された結果が書き込まれる。

        @details
        CCM モードではバイト単位で復号化処理が行われるため 1 回の呼び出しで必要になる dstSize は srcSize と同じだけ必要です。
    ...
    {
        return m_Impl.UpdateDecryption(pDst, dstSize, pSrc, srcSize);
    }
    ...
    /**
        ...
        @details
        これまでに入力された追加認証データと暗号文データから計算された MAC を取得します。

        暗号化では計算した MAC を暗号文、追加認証データとあわせて送信します。
        復号化では計算した MAC と受信した MAC との一致を確認することで暗号文、追加認証データに改ざんがないことを検出できます。
     */
...
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CcmDecryptor<T>::BlockSize);
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CcmDecryptor<T>::MaxNonceSize);
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CcmDecryptor<T>::MaxMacSize);
