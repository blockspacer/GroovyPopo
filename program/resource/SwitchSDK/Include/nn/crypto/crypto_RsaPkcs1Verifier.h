...
/**
    @file
    @brief RSA-PKCS1 署名を検証するための API の宣言
 */
...
/**
    ...
    実際の使用には @ref Rsa2048Pkcs1Sha256Verifier などを利用してください。
...
{
    NN_DISALLOW_COPY(RsaPkcs1Verifier);
...
public:
    ...
    /**
        ...
        @details
        指定された署名データがこれまで入力された検証対象のデータに対して正しい RSA-PKCS1 署名であることを検証します。
    ...
    /**
        @brief      RSA-PKCS1 署名を検証する関数です。作業領域としてスタックを利用します。
        ...
        @details
        指定された署名データが指定された検証対象のデータに対して正しい RSA-PKCS1 署名であることを検証します。
    ...
    {
        RsaPkcs1Verifier<KeySize, HashFunction> verifier;
        if (!verifier.Initialize(pModulus, modulusSize, pPublicExponent, publicExponentSize))
        {
            return false;
        }
        verifier.Update(pRawMessage, rawMessageSize);
        return verifier.Verify(pSignature, signatureSize);
    }

    /**
        @brief      RSA-PKCS1 署名を検証する関数です。作業領域として与えられたワークバッファを利用します。
                        ...
                        void* pWorkBuffer, size_t workBufferSize) NN_NOEXCEPT
    {
        ...
        verifier.Update(pRawMessage, rawMessageSize);
        return verifier.Verify(pSignature, signatureSize, pWorkBuffer, workBufferSize);
    }
...
template<size_t T, typename U> const size_t RsaPkcs1Verifier<T, U>::HashSize;
template<size_t T, typename U> const size_t RsaPkcs1Verifier<T, U>::SignatureSize;
template<size_t T, typename U> const size_t RsaPkcs1Verifier<T, U>::RequiredWorkBufferSize;
