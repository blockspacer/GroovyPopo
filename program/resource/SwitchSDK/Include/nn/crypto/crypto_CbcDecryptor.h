...
/**
    @file
    @brief CBC モードによる復号化を利用するための API の宣言
 */
...
/**
    @brief      CBC 暗号利用モードによる復号化を行うクラスです。
    ...
    実際の使用には Aes128CbcDecryptor などを利用してください。

    テンプレートとして指定するブロック暗号クラスは以下の条件を満たす実装になっている必要があります。
    各関数の引数など詳細な説明については AesDecryptor クラスを参照してください。
...
class CbcDecryptor
{
    NN_DISALLOW_COPY(CbcDecryptor<BlockCipher>);
...
public:
     ...
     */
    CbcDecryptor() NN_NOEXCEPT {}

    /**
        @brief      CBC モードで復号化の処理を初期化します。
        ...
        @pre
        - pBlockCipher で指定するインスタンスは DecryptBlock を実行できる。
        - ivSize == IvSize
        ...
        @details
        pBlockCipher には復号化に使用するブロック暗号を実装するインスタンスを指定します。
        あらかじめ鍵の設定など必要な初期化を行い、DecryptBlock が実行できる状態になっている必要があります。
    ...
    /**
        ...
        @details
        CBC モードでは復号化はブロック単位で処理されるため、1 回の呼び出しで dstSize として必要になるサイズは最大で (srcSize + ブロックサイズ - 1) になります。
...
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CbcDecryptor<T>::BlockSize);
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CbcDecryptor<T>::IvSize);
