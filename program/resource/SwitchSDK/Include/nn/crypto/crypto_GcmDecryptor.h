...
/**
    @file
    @brief GCM モードによる認証付き復号化を利用するための API の宣言
 */
...
/**
    @brief      GCM 暗号利用モードによる認証付き復号化を行うクラスです。
    ...
    -# EncryptBlock という単一ブロックを復号化する関数を持つ。
...
class GcmDecryptor
{
    NN_DISALLOW_COPY(GcmDecryptor<BlockCipher>);
...
public:
     ...
     */
    GcmDecryptor() NN_NOEXCEPT {}

    /**
        @brief      GCM モードによる認証付き復号化の処理を初期化します。
    ...
    /**
        ...
        @pre
        - インスタンスは追加認証データ処理可能状態または復号化処理可能状態である。
        - dstSize >= srcSize
        ...
        @details
        GCM モードではバイト単位で復号化処理が行われるため 1 回の呼び出しで必要になる dstSize は srcSize と同じだけ必要です。
    ...
    /**
        ...
        @pre
        - インスタンスは追加認証データ処理可能状態または復号化処理可能状態または完了状態である。
        - macSize >= MacSize
...
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t GcmDecryptor<T>::BlockSize);
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t GcmDecryptor<T>::MacSize);
