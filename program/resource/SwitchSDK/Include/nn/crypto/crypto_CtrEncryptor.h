...
/**
    @file
    @brief CTR モードによる暗号化を利用するための API の宣言
 */
...
/**
    @brief      CTR 暗号利用モードによる暗号化を行うクラスです。
    ...
    実際の使用には Aes128CtrEncryptor などを利用してください。
...
class CtrEncryptor
{
    NN_DISALLOW_COPY(CtrEncryptor<BlockCipher>);
...
public:
     ...
     */
    CtrEncryptor() NN_NOEXCEPT {}

    /**
        @brief      CTR モードで暗号化の処理を初期化します。
    ...
    /**
        ...
        @details
        CTR モードではバイト単位で暗号化処理が行われるため 1 回の呼び出しで必要になる dstSize は srcSize と同じだけ必要です。
...
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CtrEncryptor<T>::BlockSize);
NN_DEFINE_STATIC_CONSTANT(template<typename T> const size_t CtrEncryptor<T>::IvSize);
