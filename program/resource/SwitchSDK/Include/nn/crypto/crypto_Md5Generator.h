...
/**
    ...
    @note
    MD5 は脆弱性が指摘されており、過去システムとの互換性以外の用途では非推奨です。

 */
class Md5Generator
{
    NN_DISALLOW_COPY(Md5Generator);

public:
    static const size_t HashSize = 16;  //!< ハッシュ値のサイズを表す定数です。
    static const size_t BlockSize = 64; //!< ハッシュ値の計算処理単位のサイズを表す定数です。

    static const size_t Asn1ObjectIdentifierSize = 18; //!< MD5 のオブジェクト ID を DER 符号化したバイナリ列のサイズを表す定数です。
    static const uint8_t Asn1ObjectIdentifier[Asn1ObjectIdentifierSize]; //!< MD5 のオブジェクト ID を DER 符号化したバイナリ列です。
public:
    /**
...
/**
    ...
    複数に分割されたデータ列のハッシュ値を計算したい場合は Md5Generator クラスを利用してください。

    @note
    MD5 は脆弱性が指摘されており、過去システムとの互換性以外の用途では非推奨です。
 */
void   GenerateMd5Hash(void* pHash, size_t hashSize, const void* pData,
