...
/**
 *  @file
 *  @brief IconShaderでアイコンを描画するための定義です。
 *  @details 
 */

namespace nn { namespace gfx {
class TextureInfo; //!< 前方参照
}}

namespace nn { namespace mii { namespace detail {

class IconGpuBufferImpl; //!< 前方参照(非公開)
...
class IconShaderInfo; //!< 前方参照

//! @name 構造体・クラス
//! @{

/**
 * @brief アイコンテクスチャ描画用バッファ
 * @details @ref IconShader でアイコンテクスチャを描画するときに
 *          @n 必要なバッファなどをまとめたクラスです。
 *          @n このクラスは、1つのアイコンテクスチャを描画するごとに1つ必要です。
 * @attention @ref IconShader を使わない場合は不要です。
 */
class IconGpuBuffer {
    NN_DISALLOW_COPY(IconGpuBuffer);
public:
    IconGpuBuffer() NN_NOEXCEPT;
    ~IconGpuBuffer() NN_NOEXCEPT;
    ...
    /**
     * @brief メモリのサイズの取得
     * @details IconGpuBuffer生成に必要な通常メモリのサイズを取得します。
     * @return サイズを返します。
     */
    static size_t CalculateMemorySize() NN_NOEXCEPT;
    /**
     * @brief メモリのアライメントの取得
     * @details IconGpuBuffer生成に必要な通常メモリのアライメントを取得します。
     * @return アライメントを返します。
     */
    ...
    /**
     * @brief メモリプールのサイズの取得
     * @details IconGpuBuffer生成に必要なメモリプールのサイズを取得します。
     ...
     * @details IconGpuBuffer生成に必要なメモリプールのアライメントを取得します。
     * @param[in]  pDevice    デバイス
     * @pre pDevice != NULL
    ...
    //! @{
    /**
     * @brief IconGpuBufferを初期化します。
     * @attention pMemory および pMemoryPoolは、@ref IconGpuBuffer::Finalize() まで保持する必要があります。
     * @attention 初期化済みのIconGpuBufferを再初期化することは出来ません。
     *            @n @ref Finalize() してから呼び出します。
     * @param[in] pMemory          IconGpuBufferが使用するメモリ
     * @param[in] memorySize       pMemoryのサイズ
     * @param[in] pDevice          デバイス
     * @param[in] pMemoryPool      IconGpuBufferが使用するメモリプール
     * @param[in] memoryPoolOffset IconGpuBufferが使用するメモリプールのオフセット
     * @param[in] memoryPoolSize   IconGpuBufferが使用するメモリプールのサイズ
     * @return 成功するとnn::ResultSuccessを返します。
     * @pre pMemory != NULL
     *      @n && pDevice != NULL
     *      @n && pMemoryPool != NULL
     *      @n && ! @ref IconGpuBuffer::IsInitialized()
     *      @n && オフセット、サイズが正常
     * @post 成功したらIconGpuBuffer::IsInitialized() = true
     */
    ...
    /**
     * @brief IconGpuBufferを破棄します。
     * @param[in] pDevice デバイス
     * @pre IconGpuBuffer::IsInitialized() == true
     * @post IconGpuBuffer::IsInitialized() = false
     */
    ...
    /**
     * @brief 背景とのブレンドに乗算済みアルファブレンドを利用しているか取得します。
     ...
     *          @n この関数の呼び出し後に、メモリプールをフラッシュする必要があります。
     */
    void SetColorTargetAndDepthStencil(nn::gfx::Device* pDevice,
     ...
     */
    detail::IconGpuBufferImpl& GetImpl() NN_NOEXCEPT;
    const detail::IconGpuBufferImpl& GetImpl() const NN_NOEXCEPT;

    //! @}
private:
    detail::IconGpuBufferImpl* m_pImpl;
};
