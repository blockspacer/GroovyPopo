...
/**
* @file
* @brief ボーンビジビリティアニメーションに関するリソースクラスの宣言
*/
...
//! @brief ボーンビジビリティアニメーションの構造体です。
struct ResBoneVisibilityAnimData
{
...
//! @brief ボーンビジビリティアニメーションのリソースです。
class ResBoneVisibilityAnim : public nn::util::AccessorBase< ResBoneVisibilityAnimData >
{
    NN_DISALLOW_COPY(ResBoneVisibilityAnim);

public:
    static const uint32_t Signature = NN_UTIL_CREATE_SIGNATURE_4('F', 'B', 'V', 'S');
    ...
    //! BoneVisibilityAnimObj::BindFast 時には名前引きを行わない比較的高速な関連付けを行います。
    //!
    ...
    //! @brief ボーンビジビリティアニメーションの総数を取得します。
    //!
    //! @return ボーンビジビリティアニメーションの総数を返します。
    //!
