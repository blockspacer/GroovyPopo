...
typedef ::nn::util::BitFlagSet<32, NpadStyleTag>::Flag<0> NpadStyleFullKey;          //!< Switch Pro Controller をモデルとした操作スタイルの定義です。

/**
 * @brief       Npad のSwitch Pro Controller をモデルとした操作スタイル (NpadStyleFullKey) の入力状態を表す構造体です。
 *
 * @details     Switch Pro Controller をモデルとした操作スタイルで扱えるデジタルボタン ( 十字ボタン, A, B, X, Y, L, R, ZL, ZR, Plus, Minus ) と
 *              左右アナログスティックの入力状態を提供します。
 *
 *              十字ボタンは禁則処理が行われます
 *              上下または左右の同時押しが発生した場合、それぞれ上と左が優先されます
 *              attributes::IsConnected で、コントローラーの接続状態を取得できます。
 */
...
/**
 * @brief       Npad のSwitch Pro Controller をモデルとした操作スタイル (NpadStyleFullKey) の最新の入力状態を取得します。
 *
 ...
 *
 *              指定の Npad ID で NpadStyleFullKey が有効になっていない場合、入力状態は無入力（デジタルボタンの押下は無く、アナログスティックはニュートラル位置）となります。
 *              Npad ID に対して有効な操作スタイルは GetNpadStyleSet() で取得できます。
 *
 *              入力状態は、 NpadFullKeyState の構造体で取得されます。
 *              具体的に読み出せる入力状態は NpadFullKeyState のリファレンスを確認してください。
 *
 ...
 *                  - 半径は AnalogStickMax
 */
void GetNpadState(NpadFullKeyState* pOutValue, const NpadIdType& id
                  ) NN_NOEXCEPT;

/**
 * @brief       Npad のSwitch Pro Controller をモデルとした操作スタイル (NpadStyleFullKey) の入力状態を過去に遡って読み出します。
 *
 ...
 *              差分だけを利用したい場合は NpadFullKeyState::samplingNumber を参照してください。
 *
 *              指定の Npad ID がSwitch Pro Controller をモデルとした操作スタイルが有効になっていない場合、入力状態は無入力（デジタルボタンの押下は無く、アナログスティックはニュートラル位置）となります。
 *
 *              入力状態は、 NpadFullKeyState の構造体で取得されます。
 *              具体的に読み出せる入力状態は NpadFullKeyState のリファレンスを確認してください。
 *
 ...
 *                      - 半径は AnalogStickMax
 */
int GetNpadStates(NpadFullKeyState* pOutValues,
                  int count,
