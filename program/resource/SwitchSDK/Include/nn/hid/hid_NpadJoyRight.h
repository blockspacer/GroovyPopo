...
/**
 * @brief       Joy-Con (R) をモデルとした操作スタイルの定義です。
 *
 * @details     Joy-Con (R) のみを用いて操作することをモデルとした操作スタイルです。
 ...
 */
typedef ::nn::util::BitFlagSet<32, NpadStyleTag>::Flag<4> NpadStyleJoyRight;

/**
 * @brief       Npad の Joy-Con (R) をモデルとした操作スタイル (NpadStyleJoyRight) の入力状態を表す構造体です。
 *
 * @details     Joy-Con (R) の操作スタイルで扱えるデジタルボタン ( A, B, X, Y, R, ZR, Joy-Con (R) SL, Joy-Con (R) SR, Plus ) と
 *              右アナログスティックの入力状態を提供します。
 *              その他の入力状態は無入力 ( デジタルボタンの押下は無く、アナログスティックはニュートラル位置 ) となります。
 *              attributes::IsConnected または attributes::IsRightConnected で、コントローラーの接続状態を取得できます。
 */
...
/**
 * @brief       Npad の Joy-Con (R) をモデルとした操作スタイル (NpadStyleJoyRight) の最新の入力状態を取得します。
 *
 ...
 *
 *              指定の Npad ID で NpadStyleJoyRight が有効になっていない場合、入力状態は無入力（デジタルボタンの押下は無く、アナログスティックはニュートラル位置）となります。
 *              Npad ID に対して有効な操作スタイルは GetNpadStyleSet() で取得できます。
 *
 *              入力状態は、 NpadJoyRightState の構造体で取得されます。
 *              具体的に読み出せる入力状態は NpadJoyRightState のリファレンスを確認してください。
 *
 ...
 *                  - 半径は AnalogStickMax
 */
void GetNpadState(NpadJoyRightState* pOutValue, const NpadIdType& id
                  ) NN_NOEXCEPT;

/**
 * @brief       Npad の Joy-Con (R) をモデルとした操作スタイル (NpadStyleJoyRight) の入力状態を過去に遡って読み出します。
 *
 ...
 *              差分だけを利用したい場合は NpadJoyRightState::samplingNumber を参照してください。
 *
 *              指定の Npad ID で NpadStyleJoyRight が有効になっていない場合、入力状態は無入力（デジタルボタンの押下は無く、アナログスティックはニュートラル位置）となります。
 *              Npad ID に対して有効な操作スタイルは GetNpadStyleSet() で取得できます。
 *
 ...
 *                      - 半径は AnalogStickMax
 */
int GetNpadStates(NpadJoyRightState* pOutValues,
                  int count,
