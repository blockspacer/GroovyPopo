/*--------------------------------------------------------------------------------*
  Copyright (C)Nintendo All rights reserved.

  These coded instructions, statements, and computer programs contain proprietary
  information of Nintendo and/or its licensed developers and are protected by
  national and international copyright laws. They may not be disclosed to third
  parties or copied or duplicated in any form, in whole or in part, without the
  prior written consent of Nintendo.

  The content herein is highly confidential and should be handled accordingly.
 *--------------------------------------------------------------------------------*/

#pragma once

#include <nn/atk/atk_SequenceTrackAllocator.h>
#include <nn/atk/atk_MidiSequenceTrack.h>
#include <nn/atk/atk_InstancePool.h>

namespace nn {
namespace atk {
namespace detail {
namespace driver {

/* ========================================================================
        class definition
   ======================================================================== */

class MidiSequenceTrackAllocator : public SequenceTrackAllocator
{
public:
    MidiSequenceTrackAllocator() NN_NOEXCEPT {}

    virtual SequenceTrack* AllocTrack( SequenceSoundPlayer* player ) NN_NOEXCEPT NN_OVERRIDE;
    virtual void FreeTrack( SequenceTrack* track ) NN_NOEXCEPT NN_OVERRIDE;
    virtual int GetAllocatableTrackCount() const NN_NOEXCEPT NN_OVERRIDE;

    unsigned long Create( void* buffer, unsigned long size ) NN_NOEXCEPT;
    void Destroy() NN_NOEXCEPT;

private:
    InstancePool<MidiSequenceTrack> m_TrackPool;
};

} // namespace nn::atk::detail::driver
} // namespace nn::atk::detail
} // namespace nn::atk
} // namespace nn

