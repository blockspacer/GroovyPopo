...
//---------------------------------------------------------------------------
//! @internal
//! @brief  WAV バイナリストリームを入力します。
//---------------------------------------------------------------------------
class WavStreamReader : public WaveformStreamReader
{
    NN_DISALLOW_COPY(WavStreamReader);

public: // コンストラクタ
    WavStreamReader() NN_NOEXCEPT;
    explicit WavStreamReader(Stream* stream) NN_NOEXCEPT;
    virtual ~WavStreamReader() NN_NOEXCEPT NN_OVERRIDE { }
...
private:
    FndResult ReadRiffChunk() NN_NOEXCEPT;

    bool ReadImpl(void* pBuffer, size_t size) NN_NOEXCEPT;
